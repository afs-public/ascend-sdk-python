...
def test_option_orders_set_option_extra_reporting_data(
    create_sdk, enrolled_account_id, create_option_order_id
):
    s = create_sdk
    ...
    request = components.SetOptionExtraReportingDataRequestCreate(
        name=f"accounts/{enrolled_account_id}/optionOrders/{option_order_id}",
        cancel_confirmed_time=datetime.now(timezone.utc),
    )

    res = s.option_orders.set_option_extra_reporting_data(
        account_id=enrolled_account_id,
        option_order_id=option_order_id,
        set_option_extra_reporting_data_request_create=request,
    )
    assert res.http_meta is not None
    assert res.http_meta.response is not None
    assert res.http_meta.response.status_code == 200
